...
type MyMembership = TeamMember | null;
...
export default function AuctionLobbyPage() {
  ...
  const [myMembership, setMyMembership] = useState<MyMembership>(null);
  ...
  const load = useCallback(async function load() {
    ...
    if (lg) setLeague(lg as League);
    if (ps) setTeams(ps as Participant[]);
    const msList = (ms ?? []) as TeamMember[];
    setMembers(msList);
    if (user) {
